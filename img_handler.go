...
func img_editor_handler(w http.ResponseWriter, r *http.Request, args []string) {
	...
	if r.Method == POST {
		...
		mux.SeeOther(w, r, "/user/img/"+args[0])
		return
	}
	var data = defData()
	data.User = user
...
func img_upload_handler(w http.ResponseWriter, r *http.Request, args []string) {
	...
	if srcf, header, err = r.FormFile("file"); err != nil {
		...
		default:
			//不支持的图片类型
			mux.NotFound(w, "Unsupported file type: "+header.Filename)
			return
		}
		...
		if err = Db.Img.Get(nil, nil, nil, img.Md5).Struct(getimg); err == sql.ErrNoRows {
			...
			mux.SeeOther(w, r, fmt.Sprint("/user/img/", id))
			return
		} else if err == nil {
			mux.SeeOther(w, r, fmt.Sprint("/user/img/", getimg.Id))
			return
		} else {
			mux.ServerError(w, err)
			return
...
func img_remote_handler(w http.ResponseWriter, r *http.Request, args []string) {
	...
	if src == "" {
		mux.NotFound(w, "图片地址为空")
		return
	}

	var title = r.FormValue("title")
	if title == "" {
		mux.NotFound(w, "图片标题为空")
		return
	}
